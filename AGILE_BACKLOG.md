...
| **Known Issues** | 🐛 **Bug Found**: Default strategy is "balanced" instead of "minimize-income". See US-029 below. |

| ID | User Story | Story Points | Priority | Status |
|----|------------|--------------|----------|--------|
| **US-029** | **Fix Default Withdrawal Strategy to minimize-income** | **1** | **P0** | **📋 To Do** |
...
| **Acceptance Criteria** | - [ ] Default strategy in `defaultHouseholdInput` is `'minimize-income'`<br>- [ ] Prefill API fallback strategy is `'minimize-income'` (not 'balanced')<br>- [ ] Default indicator shows "(Default)" for minimize-income in results<br>- [ ] Strategy selector shows "Income Minimization (GIS-Optimized)" when page loads<br>- [ ] Smart recommendation logic still overrides for users with specific asset mixes<br>- [ ] Manual test: New user with no profile sees minimize-income<br>- [ ] Manual test: User with typical portfolio sees minimize-income |
| **Tasks** | - [ ] Change default in `/webapp/lib/types/simulation.ts` line 565 from 'corporate-optimized' to 'minimize-income'<br>- [ ] Change prefill API fallback in `/webapp/app/api/simulation/prefill/route.ts` line 480 from 'balanced' to 'minimize-income'<br>- [ ] Change prefill API default for typical retiree in `/webapp/app/api/simulation/prefill/route.ts` line 525 from 'balanced' to 'minimize-income'<br>- [ ] Test with new user profile (no assets)<br>- [ ] Test with typical mixed portfolio<br>- [ ] Verify smart recommendation still works for RRIF>40%, corporate>30%, etc.<br>- [ ] Deploy to production |
