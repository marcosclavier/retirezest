...
| ID | User Story | Story Points | Priority | Status |
|----|------------|--------------|----------|--------|
| **US-086** | **Investigate Why Users Don't Run Simulations - Missing Data Categories?** | **3** | **P1** 🟡 | **📋 To Do** |
| **Description** | As a product manager, I want to understand why users who enter some data don't run simulations, specifically investigating if they can't find the right categories in our forms (e.g., pension, rental income, part-time work, dividends) so that we can add missing categories and improve data entry UX |
| **Hypothesis** | Users ARE trying to enter data but CAN'T FIND their specific income/asset/expense category in our forms. Examples:<br>- Pension income not clearly labeled or visible<br>- Rental income category missing or unclear<br>- Part-time/consulting work doesn't have clear field<br>- Corporate dividends not separated from employment income<br>- Inheritance, gifts, or one-time income not supported<br>- Healthcare expenses, property taxes not clear<br>- Missing common Canadian asset types (LIF, LRIF, RESP) |
| **Acceptance Criteria** | **Phase 1: Data Analysis (4 hours)**<br>- [ ] Query database for users with partial data entry:<br>  - Users with 0 assets, 0 income, 0 expenses (never started)<br>  - Users with 1-2 items but didn't continue (started but stopped)<br>  - Users with assets but no income (incomplete picture)<br>  - Users with income but no expenses (can't calculate needs)<br>- [ ] Calculate completion rates for each data category:<br>  - % users with at least 1 asset<br>  - % users with at least 1 income source<br>  - % users with expenses entered<br>  - % users with retirement age set<br>  - % users with CPP/OAS ages set<br>- [ ] Identify most common "stopping points" in data entry flow<br>- [ ] Document findings in DATA_COMPLETENESS_ANALYSIS.md<br><br>**Phase 2: User Research (4 hours)**<br>- [ ] Review user feedback/support tickets for data entry issues<br>- [ ] Check for error messages or validation failures in logs<br>- [ ] Compare our data categories against competitors:<br>  - Wealthsimple retirement tool<br>  - Questrade retirement calculator<br>  - Canada.ca retirement income calculator<br>- [ ] Create list of missing categories users might need<br>- [ ] Survey users who entered partial data: "What income/asset type were you trying to add?"<br><br>**Phase 3: UI/UX Audit (2 hours)**<br>- [ ] Screenshot all data entry forms (assets, income, expenses)<br>- [ ] List all available categories/fields currently<br>- [ ] Identify confusing labels or hidden fields<br>- [ ] Check if common categories are "above the fold"<br>- [ ] Verify mobile experience (harder to find categories on small screen?)<br>- [ ] Document UX issues in report<br><br>**Phase 4: Recommendations (2 hours)**<br>- [ ] Prioritized list of missing categories to add (P0, P1, P2)<br>- [ ] UI improvements to make existing categories more discoverable<br>- [ ] Suggested help text or examples for unclear fields<br>- [ ] Create user stories for each missing category (US-087, US-088, etc.)<br>- [ ] Present findings to team with data-driven recommendations |
| **Tasks** | **Database Queries**:<br>```sql<br>-- Users with partial data<br>SELECT <br>  u.id,<br>  u.email,<br>  COUNT(DISTINCT a.id) as asset_count,<br>  COUNT(DISTINCT i.id) as income_count,<br>  COUNT(DISTINCT e.id) as expense_count,<br>  u.createdAt<br>FROM User u<br>LEFT JOIN Asset a ON a.userId = u.id<br>LEFT JOIN Income i ON i.userId = u.id<br>LEFT JOIN Expense e ON e.userId = u.id<br>WHERE u.createdAt > '2026-01-01'<br>GROUP BY u.id<br>HAVING asset_count > 0 OR income_count > 0 OR expense_count > 0;<br><br>-- Completion funnel<br>SELECT <br>  'Has any data' as stage,<br>  COUNT(*) as users<br>FROM User<br>WHERE EXISTS (SELECT 1 FROM Asset WHERE userId = User.id)<br>   OR EXISTS (SELECT 1 FROM Income WHERE userId = User.id)<br>   OR EXISTS (SELECT 1 FROM Expense WHERE userId = User.id)<br>UNION ALL<br>SELECT 'Has assets' as stage, COUNT(*) FROM User WHERE EXISTS (SELECT 1 FROM Asset WHERE userId = User.id)<br>UNION ALL<br>SELECT 'Has income' as stage, COUNT(*) FROM User WHERE EXISTS (SELECT 1 FROM Income WHERE userId = User.id)<br>UNION ALL<br>SELECT 'Has expenses' as stage, COUNT(*) FROM User WHERE EXISTS (SELECT 1 FROM Expense WHERE userId = User.id)<br>UNION ALL<br>SELECT 'Has simulation' as stage, COUNT(*) FROM User WHERE simulationCount > 0;<br>```<br><br>**User Research**:<br>- Review support@retirezest.com inbox for data entry questions<br>- Check UserFeedback table for mentions of "can't find", "missing", "where is"<br>- Survey 19 users with partial data: "What stopped you from completing your profile?"<br><br>**Competitor Analysis**:<br>- Wealthsimple: Screenshot income categories<br>- Questrade: Screenshot asset types<br>- Canada.ca: Screenshot expense categories<br>- Document categories we're missing<br><br>**Current Categories Audit**:<br>Assets: TFSA, RRSP, RRIF, Non-Reg, Cash, Property<br>Income: Employment, CPP, OAS, Pension, Other<br>Expenses: Monthly, Annual, One-Time<br><br>**Potentially Missing Categories**:<br>Assets: LIF, LRIF, RESP, Corporate account, Crypto, Precious metals<br>Income: Rental, Dividends, Capital gains, Consulting/freelance, Part-time, Disability, Inheritance<br>Expenses: Healthcare, Property tax, Insurance, Travel, Hobbies, Gifts, Long-term care |
| **Technical Notes** | **Query to Find Stopping Points**:<br>```typescript<br>// Find users who started but didn't finish<br>const partialUsers = await prisma.user.findMany({<br>  where: {<br>    simulationCount: 0,<br>    OR: [<br>      { assets: { some: {} } },<br>      { incomes: { some: {} } },<br>      { expenses: { some: {} } }<br>    ]<br>  },<br>  include: {<br>    assets: true,<br>    incomes: true,<br>    expenses: true,<br>    userFeedback: true<br>  }<br>});<br><br>// Analyze patterns<br>partialUsers.forEach(user => {<br>  console.log({<br>    email: user.email,<br>    hasAssets: user.assets.length > 0,<br>    hasIncome: user.incomes.length > 0,<br>    hasExpenses: user.expenses.length > 0,<br>    createdAt: user.createdAt,<br>    daysSinceSignup: daysSince(user.createdAt)<br>  });<br>});<br>```<br><br>**Example Missing Category Impact**:<br>If 30% of users have pension income but we don't have clear "Pension" field → they can't complete profile → they don't simulate<br><br>**Files to Check**:<br>- `webapp/app/(dashboard)/profile/assets/page.tsx` - Asset entry form<br>- `webapp/app/(dashboard)/profile/income/page.tsx` - Income entry form<br>- `webapp/app/(dashboard)/profile/expenses/page.tsx` - Expense entry form<br>- `webapp/prisma/schema.prisma` - Data model<br>- `webapp/lib/types/simulation.ts` - Simulation input types |
| **User Impact** | **CRITICAL** - If this hypothesis is correct, we're losing users not due to UX polish but due to **missing fundamental data categories**. Adding 2-3 missing categories could unlock 50%+ of stalled users. This is much higher ROI than button styling or modals. |
| **Priority Justification** | **P1 (High)** because:<br>1. **Data-driven hypothesis** - Product owner insight based on user behavior patterns<br>2. **High-impact potential** - Could unlock 19 users + future signups<br>3. **Root cause analysis** - Gets to real blocker, not symptoms<br>4. **Quick investigation** - 3 story points (~12 hours total)<br>5. **Actionable outcomes** - Will generate specific user stories for missing categories<br>6. **Prevents wasted effort** - Stops us from building wrong solutions (modals, emails) before fixing root cause |
| **Success Metrics** | **Discovery Metrics:**<br>- [ ] Data completeness report created (% users with assets, income, expenses)<br>- [ ] Stopping points identified (which category causes most drop-off)<br>- [ ] Missing categories documented (prioritized list)<br>- [ ] User feedback analyzed (quotes about what they couldn't find)<br><br>**Validation Metrics:**<br>- [ ] At least 3 missing categories identified as P0/P1<br>- [ ] At least 2 UX improvements identified for existing categories<br>- [ ] Clear correlation found between missing category and drop-off<br>- [ ] Recommendations presented to team with data backing<br><br>**Success Criteria:**<br>This story succeeds if it generates 3-5 actionable user stories (US-087+) for adding missing categories or improving data entry UX, backed by data showing these will unlock stalled users. |
| **Related Stories** | - US-060 (Add LIF/LRIF) - Example of missing asset category<br>- US-069 (Validation errors) - May reveal what users tried to enter<br>- US-071 (Re-engagement email) - Should wait until missing categories added<br>- US-085 (Simplify simulation UX) - Related to overall UX improvements |
| **Next Steps After This Story** | Based on findings, create user stories like:<br>- US-087: Add "Pension Income" category with clear labeling<br>- US-088: Add "Rental Income" to income types<br>- US-089: Add "Part-time/Consulting Work" income field<br>- US-090: Add "Healthcare Expenses" category<br>- US-091: Improve income source labels and help text<br>- US-092: Add "Other Assets" with examples (RESP, LIF, etc.) |
...
| ID | User Story | Story Points | Priority | Status |
|----|------------|--------------|----------|--------|
| **US-040** | **Investment Timeline Display - Show Account Lifespan & Key Dates** | **5** | **P1** | **📋 To Do** |
...
| ID | User Story | Story Points | Priority | Status |
|----|------------|--------------|----------|--------|
| **US-041** | **Explain Compound Growth - Why Balances Grow Despite Withdrawals** | **3** | **P2** | **📋 To Do** |
| **Description** | As a user, I want to understand why my investment balances might grow (e.g., $300K → $400K) even though I'm withdrawing money every year so that I can trust the simulation results and understand compound interest |
...
| ID | User Story | Story Points | Priority | Status |
|----|------------|--------------|----------|--------|
| **US-085** | **Review and Simplify Simulation UX - Reduce Information Overload** | **5** | **P1** | **📋 To Do** |
| **Description** | As a user, I want the simulation results page to be simpler and less overwhelming so that I can quickly understand my retirement outlook without being confused by too much information or unnecessary controls |
| **User Concerns** | **Key Questions to Address:**<br>1. **"Is the slider needed?"** - Evaluate whether What-If sliders add value or create confusion<br>2. **"We have health score and simulation score - do we have too much information?"** - Assess if dual scoring systems are redundant or overwhelming<br>3. **"How can we make the information simpler to show?"** - Identify opportunities to streamline the UI and focus on key insights |
| **Acceptance Criteria** | **UX Audit Phase:**<br>- [ ] Complete heuristic evaluation of simulation results page<br>- [ ] Document all UI elements currently shown (sliders, scores, charts, tables, metrics)<br>- [ ] Identify information hierarchy issues (what's most important vs. what's secondary)<br>- [ ] Survey users about which elements they find most/least useful<br>- [ ] Analyze user session recordings to see which elements users interact with<br>- [ ] Compare with competitor retirement planning tools (simplicity benchmarks)<br><br>**Slider Evaluation:**<br>- [ ] Assess whether What-If sliders are discoverable and understood<br>- [ ] Measure slider usage rate (% of users who interact with sliders)<br>- [ ] Determine if sliders add confusion vs. value<br>- [ ] Consider alternatives: Separate "What-If" mode, remove sliders, or improve clarity<br>- [ ] Decision documented: Keep, remove, or redesign sliders<br><br>**Scoring System Evaluation:**<br>- [ ] Analyze difference between health score and simulation score<br>- [ ] Determine if users understand the distinction<br>- [ ] Assess if both scores are necessary or if one would suffice<br>- [ ] Consider consolidating into single "Retirement Readiness Score"<br>- [ ] User testing to validate scoring approach<br>- [ ] Decision documented: Keep both, merge, or redesign<br><br>**Simplification Phase:**<br>- [ ] Create "above the fold" priority list (3-5 key metrics users MUST see)<br>- [ ] Move secondary information to expandable sections or tabs<br>- [ ] Reduce visual clutter (whitespace, font sizes, color usage)<br>- [ ] Implement progressive disclosure (show simple view first, advanced details on demand)<br>- [ ] Create mobile-first responsive design (forces simplification)<br>- [ ] A/B test simplified vs. current design<br><br>**Implementation:**<br>- [ ] Redesign simulation results page based on findings<br>- [ ] Implement simplified UI components<br>- [ ] Update documentation and help text<br>- [ ] User acceptance testing (UAT) with 10+ users<br>- [ ] Measure improvement in user comprehension and satisfaction |
| **Tasks** | **Week 1: Discovery & Research (8 hours)**<br>- [ ] **UX Audit** (3 hours):<br>  - Screenshot current simulation results page<br>  - List all UI elements (sliders, scores, charts, tables, metrics, buttons)<br>  - Categorize by importance (critical, helpful, nice-to-have, unnecessary)<br>  - Identify information overload issues<br>- [ ] **User Research** (3 hours):<br>  - Create survey: "Which elements do you find most useful?"<br>  - Send to 50-100 users via email<br>  - Review session recordings (Hotjar, FullStory, or similar)<br>  - Analyze which elements users click/hover/ignore<br>  - Interview 3-5 users for qualitative feedback<br>- [ ] **Competitive Analysis** (2 hours):<br>  - Review Wealthsimple, Questrade, Moneysense retirement calculators<br>  - Benchmark simplicity and information hierarchy<br>  - Document best practices and inspiration<br><br>**Week 2: Analysis & Recommendations (6 hours)**<br>- [ ] **Slider Analysis** (2 hours):<br>  - Query analytics: % of users who interact with sliders<br>  - Identify which sliders are used most/least<br>  - User comprehension test: Do users understand what sliders do?<br>  - Document recommendation: Keep, remove, or redesign<br>- [ ] **Scoring System Analysis** (2 hours):<br>  - Document difference between health score and simulation score<br>  - User comprehension test: Can users explain the difference?<br>  - Assess redundancy and confusion potential<br>  - Explore single unified score concept<br>  - Document recommendation: Keep both, merge, or redesign<br>- [ ] **Information Architecture** (2 hours):<br>  - Create "above the fold" priority list (3-5 key metrics)<br>  - Design progressive disclosure approach (simple → advanced)<br>  - Sketch wireframes for simplified layout<br>  - Present recommendations to team for approval<br><br>**Week 3: Design & Prototyping (8 hours)**<br>- [ ] **Design New Layout** (4 hours):<br>  - Create Figma mockups for simplified simulation results<br>  - "Quick View" mode: 3-5 key metrics + health score + action items<br>  - "Detailed View" mode: Charts, tables, year-by-year breakdown<br>  - Implement tabbed or expandable sections for optional detail<br>  - Mobile-responsive design<br>- [ ] **User Testing** (2 hours):<br>  - Create Figma prototype<br>  - Test with 5-10 users (UserTesting.com or in-person)<br>  - Measure comprehension: "What is your retirement outlook?"<br>  - Measure satisfaction: "Is this easier to understand?"<br>  - Iterate based on feedback<br>- [ ] **Finalize Design** (2 hours):<br>  - Incorporate user feedback<br>  - Get stakeholder approval<br>  - Create implementation spec for developers<br><br>**Week 4: Implementation (8 hours)**<br>- [ ] **Frontend Development** (6 hours):<br>  - Update `webapp/app/(dashboard)/simulation/page.tsx`<br>  - Implement simplified layout components<br>  - Progressive disclosure (tabs, accordions, or "Show More" buttons)<br>  - Conditional rendering based on user preference<br>  - Responsive mobile design<br>- [ ] **Testing & Deployment** (2 hours):<br>  - Test on desktop, tablet, mobile<br>  - A/B test: 50% see new design, 50% see old design<br>  - Monitor user satisfaction scores<br>  - Gradual rollout if successful |
| **Technical Notes** | **Current Simulation Results Elements** (to audit):<br>1. **Scores/Metrics**:<br>   - Health Score (0-100)<br>   - Simulation Score (0-100)<br>   - Success Rate (%)<br>   - Years of Success<br>   - Final Estate Value<br>   - Withdrawal Strategy Name<br><br>2. **What-If Sliders** (if present):<br>   - Spending adjustment slider<br>   - Retirement age slider<br>   - Asset return slider<br>   - Inflation slider<br><br>3. **Charts/Visualizations**:<br>   - Net Worth Over Time (line chart)<br>   - Income vs. Expenses (stacked area chart)<br>   - Tax Burden (bar chart)<br>   - Account Balances (line chart)<br>   - Cash Flow (waterfall chart)<br><br>4. **Tables**:<br>   - Year-by-year breakdown<br>   - Income sources by year<br>   - Withdrawals by account<br>   - Tax details<br><br>5. **Action Items/Recommendations**:<br>   - Optimization suggestions<br>   - Risk warnings<br>   - Educational tips<br><br>**Simplification Ideas to Consider:**<br><br>**Option 1: Single Unified Score**<br>- Merge health score + simulation score → "Retirement Readiness Score"<br>- 0-100 scale with clear explanation<br>- Color-coded: Red (<60), Yellow (60-79), Green (80+)<br>- One number users remember instead of two<br><br>**Option 2: Remove or Redesign Sliders**<br>- If low usage (<20%), consider removing<br>- Alternative: Separate "What-If Scenarios" page<br>- Or: One prominent slider for most impactful variable (retirement age)<br>- Hide other sliders in "Advanced What-If" section<br><br>**Option 3: Progressive Disclosure**<br>```tsx<br>// Simple View (Default)<br><div className="simple-view"><br>  <h2>Your Retirement Outlook</h2><br>  <div className="score-card"><br>    <div className="score">85/100</div><br>    <div className="verdict">Looking Good!</div><br>  </div><br>  <div className="key-metrics"><br>    <Metric label="Success Rate" value="96%" /><br>    <Metric label="Years Covered" value="30 years" /><br>    <Metric label="Estate Value" value="$420K" /><br>  </div><br>  <button onClick={() => setShowDetails(true)}><br>    Show Detailed Analysis<br>  </button><br></div><br><br>// Detailed View (Expandable)<br>{showDetails && (<br>  <div className="detailed-view"><br>    <Tabs><br>      <Tab label="Charts">...</Tab><br>      <Tab label="Year-by-Year">...</Tab><br>      <Tab label="Tax Details">...</Tab><br>      <Tab label="What-If">...</Tab><br>    </Tabs><br>  </div><br>)}<br>```<br><br>**Option 4: Mobile-First Design**<br>- Design for mobile screen first (forces prioritization)<br>- Show only 3-5 most important metrics above fold<br>- Everything else in tabs or expandable sections<br>- Desktop gets same layout (consistency + simplicity)<br><br>**Analytics to Track:**<br>```javascript<br>// What-If Slider Usage<br>SELECT <br>  COUNT(DISTINCT user_id) as total_users,<br>  COUNT(DISTINCT CASE WHEN slider_interaction THEN user_id END) as slider_users,<br>  (slider_users / total_users * 100) as slider_usage_rate<br>FROM simulation_sessions;<br><br>// Element Visibility/Interaction<br>// Use Hotjar, Mixpanel, or GA4 to track:<br>// - Which sections users scroll to<br>// - Which tabs users click<br>// - Time spent on each section<br>// - Bounce rate from results page<br>```<br><br>**Files to Modify:**<br>- `webapp/app/(dashboard)/simulation/page.tsx` - Main results page<br>- `webapp/components/simulation/HealthScore.tsx` - Scoring component<br>- `webapp/components/simulation/WhatIfSliders.tsx` - Slider controls<br>- `webapp/components/simulation/ResultsSummary.tsx` - Summary card<br>- `webapp/components/simulation/Charts.tsx` - Data visualizations |
| **User Impact** | **High** - Simulation results page is the core value proposition. If users find it confusing or overwhelming, they won't understand their retirement outlook and may abandon the tool. Simplification directly improves user comprehension, satisfaction, and retention. |
| **Priority Justification** | **P1 (High)** because:<br>1. **User feedback driven** - You specifically raised concerns about complexity<br>2. **Core UX issue** - Results page is where users get value<br>3. **Information overload** - Dual scoring + sliders + charts may overwhelm users<br>4. **Quick ROI** - UX improvements benefit all users immediately<br>5. **Competitive advantage** - Simplicity is a differentiator in retirement planning tools<br>6. **Retention risk** - Confused users don't become long-term users |
| **Success Metrics** | **Comprehension Metrics:**<br>- [ ] 80%+ of users can explain their retirement outlook after viewing results<br>- [ ] Time to comprehension decreases by 30% (measured via user testing)<br>- [ ] User satisfaction with results page increases from current to >4.5/5<br><br>**Engagement Metrics:**<br>- [ ] Bounce rate from results page decreases by 20%<br>- [ ] Time on results page increases (indicates users are reading, not confused)<br>- [ ] Repeat simulation rate increases (users feel confident using tool)<br><br>**Behavioral Metrics:**<br>- [ ] Support questions about "how to read results" decrease by 40%<br>- [ ] Users share/export results 30% more often (confidence in understanding)<br>- [ ] Conversion to premium increases (users see value more clearly)<br><br>**A/B Test Success Criteria:**<br>- [ ] Simplified design preferred by 60%+ of users<br>- [ ] Comprehension scores higher with simplified design<br>- [ ] No decrease in engagement with advanced features (for power users) |
| **Dependencies** | - Analytics tool (Mixpanel, Amplitude, or GA4) to measure usage patterns<br>- User research tool (UserTesting.com, Hotjar, or similar) for qualitative feedback<br>- Design tool (Figma) for prototyping<br>- A/B testing capability (optional but recommended) |
| **Related Stories** | - US-040 (Investment Timeline) - Both focus on improving results comprehension<br>- US-041 (Explain Compound Growth) - Both address user understanding<br>- US-027 (Educational Guidance) - May reduce need for complex UI if education is good<br>- US-028 (Help Center) - Good docs can enable more complex UI, but simplicity is better |
| **Open Questions** | **Questions to Answer During Discovery:**<br>1. What % of users actually use the What-If sliders?<br>2. Can users explain the difference between health score and simulation score?<br>3. Which charts/metrics do users find most valuable?<br>4. Which elements do users ignore completely?<br>5. Do users prefer simple summary or detailed breakdown?<br>6. How does mobile usage compare to desktop (mobile forces simplicity)?<br>7. What do competitor tools show on results page (best practices)?<br>8. What is the optimal number of metrics to show "above the fold"? |

**Total Story Points**: 15 → 20 → 28 → 33 → 38 → 40 → 45
**User Stories**: US-008 (✅), US-009, US-010 (✅), US-025 (✅), US-026 (✅), US-027, US-028, US-029 (✅), US-038, US-039, US-042, US-049, US-085
...
| ID | User Story | Story Points | Priority | Status |
|----|------------|--------------|----------|--------|
| **US-075** | **Connect Real Estate Downsizing UI to Simulation Engine** | **5** | **P0** 🔴 | **📋 To Do** |
...
| **Examples** | **Example 1: Downsizing from $600K to $300K**<br>User Input (UI):<br>- Property value: $600,000<br>- Plan to sell: Yes<br>- Sale year: 2031 (age 70)<br>- Downsize to: $300,000<br>- Is principal residence: Yes<br><br>Expected Simulation Result:<br>- Age 70 (2031): Property sale proceeds $300K added to non-registered account<br>- Age 70+: No mortgage payments (was $0 already)<br>- Age 70+: Rental income stops if property was generating rental income<br>- Capital gains tax: $0 (principal residence exemption)<br><br>Current Simulation Result:<br>- ❌ Property sale IGNORED<br>- ❌ $300K proceeds NEVER appear in cash flow<br>- ❌ User retirement projection is INCORRECT<br><br>**Example 2: Selling Rental Property**<br>User Input (UI):<br>- Rental property value: $500,000<br>- Purchase price: $300,000<br>- Plan to sell: Yes<br>- Sale year: 2035 (age 74)<br>- Downsize to: $0 (selling outright)<br>- Is principal residence: No (investment property)<br>- Monthly rental income: $2,000<br><br>Expected Simulation Result:<br>- Ages 65-73: Rental income $24,000/year<br>- Age 74 (2035): Property sale proceeds added to investments<br>- Age 74: Capital gains = $200K, Taxable gain = $100K (50%), Tax = ~$40K<br>- Age 74: Net proceeds = $500K - $40K = $460K added to non-registered<br>- Age 75+: Rental income = $0 (property sold)<br><br>Current Simulation Result:<br>- ✅ Rental income $24,000/year (works via other_incomes array)<br>- ❌ Property sale IGNORED<br>- ❌ Rental income continues forever (US-074 bug)<br>- ❌ Capital gains NEVER calculated<br>- ❌ User projection is INCORRECT |
...
| ID | User Story | Story Points | Priority | Status |
|----|------------|--------------|----------|--------|
| **US-080** | **Fix RRIF Minimum Withdrawal Enforcement** | **5** | **P0** 🔴 | **📋 To Do** |
| **Description** | As a user with RRSP/RRIF accounts who is age 72+, I want the simulation to enforce mandatory RRIF minimum withdrawals as required by CRA so that my retirement projections comply with Canadian tax law and accurately reflect required withdrawals |
| **Bug Report** | **Discovery**: February 5, 2026 - Comprehensive calculation validation testing<br>**Issue**: Simulation does NOT enforce mandatory RRIF minimum withdrawals after age 72<br>**Impact**: Affects ALL 6 test accounts consistently across ages 72-95<br>**Evidence**: Account with $1.4M RRIF balance at age 72 shows NO minimum withdrawal enforcement<br>**Root Cause**: `simulate_year()` function in `modules/simulation.py` likely not checking for RRIF minimum withdrawal requirements<br>**Legal Compliance**: CRA requires minimum RRIF withdrawals starting at age 72 (year after RRSP converted to RRIF at 71) |
| **Acceptance Criteria** | - [ ] RRIF minimum withdrawals enforced for all ages 72+<br>- [ ] Minimum percentage based on CRA table (e.g., 7.48% at age 72, 8.75% at age 80, 20% at age 95)<br>- [ ] Minimum withdrawal occurs EVEN IF spending needs are already met from other sources<br>- [ ] Minimum withdrawal added to taxable income<br>- [ ] If withdrawal strategies need more than minimum, use strategy amount (minimum is a floor, not ceiling)<br>- [ ] Test case: Account age 72 with $1M RRIF balance → minimum withdrawal $74,800 (7.48%)<br>- [ ] Test case: Account age 95 with $400K RRIF balance → minimum withdrawal $80,000 (20%)<br>- [ ] All 6 test accounts pass RRIF minimum withdrawal validation<br>- [ ] No regression - other withdrawal logic still works correctly |
| **Tasks** | - [ ] **Locate RRIF Withdrawal Logic** (30 min):<br>  - Open `juan-retirement-app/modules/simulation.py`<br>  - Find `simulate_year()` function<br>  - Locate RRIF withdrawal section<br>  - Identify where minimum withdrawal check should be added<br>- [ ] **Implement Minimum Withdrawal Check** (2 hours):<br>  - Add CRA RRIF minimum percentage table (ages 71-95+)<br>  - Calculate minimum required withdrawal: `rrif_balance * minimum_percentage`<br>  - Before withdrawal strategy logic, check if minimum required<br>  - If age >= 72 and RRIF balance > 0:<br>    - Calculate minimum withdrawal<br>    - Set withdrawal amount to MAX(strategy_withdrawal, minimum_withdrawal)<br>  - Add minimum withdrawal to taxable income<br>  - Update RRIF balance after withdrawal<br>  - Track minimum withdrawal separately in year results (for transparency)<br>- [ ] **Test Implementation** (1.5 hours):<br>  - Re-run calculation validation tests on all 6 accounts<br>  - Verify RRIF minimum withdrawals present at age 72+<br>  - Verify withdrawal amounts meet or exceed minimums<br>  - Verify no regression in other withdrawal logic<br>  - Test edge cases:<br>    - Age 71 (first RRIF year, minimum = 0 or pro-rated)<br>    - RRIF balance very low (< $1000)<br>    - RRIF depleted mid-retirement<br>    - Multiple RRIF accounts (Person 1 + Person 2)<br>- [ ] **Documentation** (30 min):<br>  - Document RRIF minimum percentage table in code comments<br>  - Update API documentation to explain minimum withdrawal enforcement<br>  - Add note to withdrawal strategy docs: "RRIF minimums enforced regardless of strategy"<br>- [ ] **Deploy & Verify** (30 min):<br>  - Deploy Python backend to production<br>  - Re-run production simulations for affected users<br>  - Monitor for any calculation errors |
| **Technical Notes** | **CRA RRIF Minimum Percentage Table (2026):**<br>```python<br>RRIF_MINIMUM_PERCENTAGES = {<br>    71: 5.28,   # First year after conversion from RRSP<br>    72: 7.48,<br>    73: 7.59,<br>    74: 7.71,<br>    75: 7.85,<br>    76: 7.99,<br>    77: 8.15,<br>    78: 8.33,<br>    79: 8.53,<br>    80: 8.75,<br>    81: 8.99,<br>    82: 9.27,<br>    83: 9.58,<br>    84: 9.93,<br>    85: 10.33,<br>    86: 10.79,<br>    87: 11.33,<br>    88: 11.96,<br>    89: 12.71,<br>    90: 13.62,<br>    91: 14.73,<br>    92: 16.12,<br>    93: 17.92,<br>    94: 20.00,<br>    95: 20.00,  # Stays at 20% for ages 95+<br>}<br>```<br><br>**Proposed Implementation (simulation.py):**<br>```python<br>def enforce_rrif_minimum_withdrawal(person, age, rrif_balance, strategy_withdrawal):<br>    """<br>    Enforce CRA RRIF minimum withdrawal requirements.<br>    Returns max(strategy_withdrawal, minimum_required).<br>    """<br>    # No minimum before age 72<br>    if age < 72:<br>        return strategy_withdrawal<br>    <br>    # No balance, no withdrawal<br>    if rrif_balance <= 0:<br>        return 0.0<br>    <br>    # Get minimum percentage from CRA table<br>    min_percentage = RRIF_MINIMUM_PERCENTAGES.get(age, 20.0) / 100.0<br>    minimum_withdrawal = rrif_balance * min_percentage<br>    <br>    # Return greater of strategy withdrawal or minimum required<br>    return max(strategy_withdrawal, minimum_withdrawal)<br>```<br><br>**Integration into simulate_year():**<br>```python<br># In simulate_year() function, after calculating strategy withdrawal:<br><br># Person 1 RRIF withdrawal<br>if person1.rrif_balance > 0:<br>    strategy_withdrawal_p1 = calculate_withdrawal_from_rrif(person1, ...)<br>    <br>    # ENFORCE MINIMUM<br>    actual_withdrawal_p1 = enforce_rrif_minimum_withdrawal(<br>        person1, <br>        current_age_p1, <br>        person1.rrif_balance, <br>        strategy_withdrawal_p1<br>    )<br>    <br>    # Update balance and taxable income<br>    person1.rrif_balance -= actual_withdrawal_p1<br>    taxable_income_p1 += actual_withdrawal_p1<br>    <br>    # Track minimum withdrawal flag for transparency<br>    rrif_minimum_enforced_p1 = (actual_withdrawal_p1 > strategy_withdrawal_p1)<br>```<br><br>**Example - Age 72 with $1M RRIF:**<br>```<br>Strategy says: Withdraw $30,000 (minimize income strategy)<br>CRA Minimum: $1,000,000 * 7.48% = $74,800<br>ACTUAL WITHDRAWAL: $74,800 (minimum enforced)<br>Taxable Income: +$74,800<br>```<br><br>**Example - Age 95 with $400K RRIF:**<br>```<br>Strategy says: Withdraw $50,000<br>CRA Minimum: $400,000 * 20% = $80,000<br>ACTUAL WITHDRAWAL: $80,000 (minimum enforced)<br>Taxable Income: +$80,000<br>``` |
| **User Impact** | **CRITICAL - Legal Compliance Issue**<br>- Affects ALL users age 72+ with RRIF accounts<br>- Current simulations are NON-COMPLIANT with CRA requirements<br>- Tax planning accuracy severely affected (underestimating withdrawals and taxes)<br>- Withdrawal strategy optimization may be completely incorrect<br>- Users could face CRA penalties if they follow simulation advice and fail to take minimums<br>- Identified in all 6 test accounts (100% failure rate) |
| **Priority Justification** | **P0 (Critical)** because:<br>1. **Legal compliance** - CRA mandates RRIF minimums, not optional<br>2. **100% failure rate** - All 6 test accounts failed this validation<br>3. **Core functionality** - RRIF withdrawals are fundamental to retirement planning<br>4. **Tax accuracy** - Underestimating withdrawals leads to underestimating taxes<br>5. **User trust** - Non-compliant advice damages credibility<br>6. **Quick fix** - 5 story points, ~4.5 hours total |
| **Success Metrics** | - [ ] All 6 test accounts pass RRIF minimum withdrawal validation (0% → 100%)<br>- [ ] RRIF withdrawals at age 72+ meet or exceed CRA minimums<br>- [ ] Taxable income increases appropriately when minimums enforced<br>- [ ] No regression - other account withdrawals (TFSA, Non-Reg) still work<br>- [ ] Documentation clearly explains minimum enforcement |
| **Related Stories** | - US-013 (RRIF Strategy Validation) - Related to RRIF calculations<br>- US-036 (RRIF Tax Withholding) - Related to RRIF withdrawals<br>- US-081, US-082 (CPP/OAS caps) - Other government benefit calculation issues<br>- US-077 (Exponential growth bug) - Similar calculation accuracy issue |
| **Test Evidence** | **Source**: CALCULATION_VALIDATION_TEST_RESULTS.md (February 5, 2026)<br><br>**Example Failure (test@example.com):**<br>- Year 2053 (Age 72): RRIF Balance $1,429,193<br>  - Expected Minimum: $106,943 (7.48%)<br>  - Actual Withdrawal: Unknown (likely < minimum)<br>  - Result: ❌ FAIL - No minimum enforced<br><br>- Year 2076 (Age 95): RRIF Balance $414,363<br>  - Expected Minimum: $82,873 (20%)<br>  - Actual Withdrawal: Unknown (likely < minimum)<br>  - Result: ❌ FAIL - Still not enforcing minimum<br><br>**Affected Accounts**: 6/6 (100%)<br>- test@example.com - RRIF $1.4M at age 72, no minimum<br>- claire.conservative@test.com - RRIF $523K at age 72, no minimum<br>- alex.aggressive@test.com - RRIF $1.1M at age 72, no minimum<br>- mike.moderate@test.com - RRIF $494K at age 72, no minimum<br>- sarah.struggling@test.com - RRIF depleted by age 73 (only 2 years affected)<br>- helen.highincome@test.com - RRIF $1.2M at age 72, no minimum |

| ID | User Story | Story Points | Priority | Status |
|----|------------|--------------|----------|--------|
| **US-081** | **Cap CPP Benefits at Legislated Maximum** | **3** | **P1** 🟡 | **📋 To Do** |
| **Description** | As a user receiving CPP benefits in retirement, I want CPP amounts capped at realistic legislated maximums so that my long-term retirement projections don't overestimate government benefits |
| **Bug Report** | **Discovery**: February 5, 2026 - Comprehensive calculation validation testing<br>**Issue**: CPP benefits grow beyond legislated maximum amounts after many years of inflation indexing<br>**Impact**: Affects ALL 6 test accounts in later retirement years (age 71+)<br>**Evidence**: test@example.com shows CPP = $20,081 in 2052 (age 71), exceeds 2025 maximum ~$17,000<br>**Further Evidence**: CPP = $32,299 in 2076 (age 95), far exceeds any reasonable maximum<br>**Root Cause**: CPP inflation indexing likely not capped at legislated maximum benefit amount |
| **Acceptance Criteria** | - [ ] CPP benefits capped at legislated maximum for each year<br>- [ ] Maximum benefit amount indexed to inflation each year (e.g., 2% per year)<br>- [ ] Cap applied AFTER inflation indexing (inflate first, then cap)<br>- [ ] Tax configuration includes CPP maximum benefit per year<br>- [ ] Historical maximums documented (2025: ~$17,000, 2026: ~$17,340, etc.)<br>- [ ] Test case: CPP starting at $15,000 in 2025 → max $17,000 in 2025, max $17,340 in 2026<br>- [ ] Test case: CPP cannot exceed ~$28,000 in 2050 (assuming 2% annual cap increase)<br>- [ ] All 6 test accounts pass CPP maximum validation<br>- [ ] No regression - CPP amounts below maximum remain unchanged |
| **Tasks** | - [ ] **Research CPP Maximums** (1 hour):<br>  - Find CRA/Service Canada CPP maximum benefit amounts for 2025-2026<br>  - Document historical maximum amounts (2020-2025)<br>  - Determine indexing rate for maximum (typically matches inflation, ~2%/year)<br>  - Research if maximum can grow faster than inflation (legislation changes)<br>- [ ] **Update Tax Configuration** (1 hour):<br>  - Add `cpp_max_annual_benefit` to tax_config_canada_2025.json<br>  - Add `cpp_max_benefit_indexing_rate` (default 2.0%)<br>  - Document assumptions in tax config<br>- [ ] **Implement CPP Cap** (1 hour):<br>  - Open `juan-retirement-app/modules/simulation.py`<br>  - Locate CPP benefit calculation<br>  - After applying inflation indexing, apply maximum cap<br>  - Formula: `cpp_benefit = min(cpp_after_inflation, cpp_max_for_year)`<br>  - Calculate `cpp_max_for_year` based on base year maximum + inflation<br>- [ ] **Test Implementation** (1 hour):<br>  - Re-run calculation validation tests on all 6 accounts<br>  - Verify CPP benefits never exceed maximum in any year<br>  - Verify CPP amounts reasonable in 2050, 2070, 2090<br>  - Test edge cases:<br>    - User starts CPP at maximum ($17K in 2025)<br>    - User starts CPP below maximum ($10K)<br>    - User defers CPP to age 70 (36% increase, may still hit cap)<br>- [ ] **Documentation** (30 min):<br>  - Document CPP maximum amounts in tax_config comments<br>  - Update API documentation<br>  - Add note to CPP calculation docs |
| **Technical Notes** | **CPP Maximum Benefit Amounts (Service Canada):**<br>```<br>Year 2025: $17,033 (age 65 start)<br>Year 2024: $16,375<br>Year 2023: $15,679<br>Year 2022: $14,445<br>Year 2021: $14,110<br><br>Average annual increase: ~6% (2021-2025)<br>Conservative estimate: ~2-3% per year going forward<br>```<br><br>**Tax Config Update (tax_config_canada_2025.json):**<br>```json<br>{<br>  "cpp": {<br>    "max_annual_benefit_2025": 17033,<br>    "max_benefit_indexing_rate": 2.0,<br>    "notes": "CPP maximum benefit is indexed annually. Historical average ~6%, using conservative 2% for projections."<br>  }<br>}<br>```<br><br>**Proposed Implementation (simulation.py):**<br>```python<br>def calculate_cpp_with_cap(base_cpp, start_year, current_year, tax_config):<br>    """<br>    Calculate CPP benefit with inflation indexing AND maximum cap.<br>    """<br>    # Get base maximum and indexing rate from config<br>    cpp_max_2025 = tax_config['cpp']['max_annual_benefit_2025']<br>    indexing_rate = tax_config['cpp']['max_benefit_indexing_rate'] / 100.0<br>    <br>    # Calculate years since 2025<br>    years_since_2025 = current_year - 2025<br>    <br>    # Calculate maximum for current year (compound indexing)<br>    cpp_max_current_year = cpp_max_2025 * ((1 + indexing_rate) ** years_since_2025)<br>    <br>    # Apply inflation to base CPP (existing logic)<br>    cpp_after_inflation = apply_inflation_to_cpp(base_cpp, start_year, current_year)<br>    <br>    # Apply cap<br>    cpp_capped = min(cpp_after_inflation, cpp_max_current_year)<br>    <br>    return cpp_capped<br>```<br><br>**Example - CPP Starting at $15K in 2025:**<br>```<br>2025: $15,000 (< $17,033 max) → $15,000<br>2030: $15,000 * 1.02^5 = $16,561 (< $18,806 max) → $16,561<br>2050: $15,000 * 1.02^25 = $24,600 (> $27,700 max) → $27,700 (CAPPED)<br>2070: $15,000 * 1.02^45 = $36,500 (> $41,200 max) → $41,200 (CAPPED)<br>```<br><br>**Example - CPP Starting at Maximum $17K in 2025:**<br>```<br>2025: $17,000 (= $17,033 max) → $17,033 (at cap)<br>2030: $17,000 * 1.02^5 = $18,765 (< $18,806 max) → $18,765<br>2050: $17,000 * 1.02^25 = $27,880 (> $27,700 max) → $27,700 (CAPPED)<br>2070: $17,000 * 1.02^45 = $41,367 (> $41,200 max) → $41,200 (CAPPED)<br>```<br><br>**Files to Modify:**<br>- `juan-retirement-app/tax_config_canada_2025.json` - Add CPP maximum<br>- `juan-retirement-app/modules/simulation.py` - Implement CPP cap logic<br>- `juan-retirement-app/docs/CALCULATION_NOTES.md` - Document CPP cap |
| **User Impact** | **MEDIUM - Long-term Accuracy**<br>- Primarily affects long-term projections (20+ years into retirement)<br>- Overestimating CPP leads to underestimating need to withdraw from savings<br>- Could result in users running out of money if they rely on inflated CPP projections<br>- Particularly problematic for users planning 30-40 year retirements (retire at 55-60) |
| **Priority Justification** | **P1 (High)** because:<br>1. **Data accuracy** - Projections should use realistic government benefit amounts<br>2. **100% affected** - All 6 test accounts show CPP exceeding maximums in later years<br>3. **Quick fix** - 3 story points, ~4 hours total<br>4. **Low risk** - Only affects CPP calculation, isolated change<br>5. **Complements US-082** - Similar fix needed for OAS |
| **Success Metrics** | - [ ] All 6 test accounts pass CPP maximum validation (0% → 100%)<br>- [ ] CPP benefits in year 2050 reasonable (< $30,000)<br>- [ ] CPP benefits in year 2070 reasonable (< $45,000)<br>- [ ] No regression - CPP amounts below maximum unchanged<br>- [ ] Documentation clearly explains CPP maximum and indexing |
| **Related Stories** | - US-082 (Cap OAS at Maximum) - Same issue, same solution pattern<br>- US-038 (CPP/OAS Income Timing Bug) - Related to CPP calculations<br>- US-080 (RRIF Minimums) - Other calculation accuracy issue |
| **Test Evidence** | **Source**: CALCULATION_VALIDATION_TEST_RESULTS.md (February 5, 2026)<br><br>**Example Failure (test@example.com):**<br>- Year 2052 (Age 71): CPP = $20,081<br>  - Expected Maximum (2052): ~$27,700 (2025 $17K * 1.02^27)<br>  - Result: ⚠️ WARN - Exceeds 2025 maximum, but within projected 2052 maximum<br>  - Needs verification against realistic future maximums<br><br>- Year 2076 (Age 95): CPP = $32,299<br>  - Expected Maximum (2076): ~$41,200 (2025 $17K * 1.02^51)<br>  - Result: ⚠️ WARN - Exceeds 2025 maximum, likely within projected 2076 maximum<br>  - May be reasonable IF maximum grows at 2% for 51 years<br><br>**Affected Accounts**: 6/6 (100%) in later retirement years |

| ID | User Story | Story Points | Priority | Status |
|----|------------|--------------|----------|--------|
| **US-082** | **Cap OAS Benefits at Legislated Maximum** | **3** | **P1** 🟡 | **📋 To Do** |
| **Description** | As a user receiving OAS benefits in retirement, I want OAS amounts capped at realistic legislated maximums so that my long-term retirement projections don't overestimate government benefits |
| **Bug Report** | **Discovery**: February 5, 2026 - Comprehensive calculation validation testing<br>**Issue**: OAS benefits grow beyond legislated maximum amounts after many years of inflation indexing<br>**Impact**: Affects ALL 6 test accounts in later retirement years (age 69+)<br>**Evidence**: test@example.com shows OAS = $12,063 in 2050 (age 69), exceeds 2025 maximum ~$8,500<br>**Further Evidence**: OAS = $20,187 in 2076 (age 95), unrealistically high<br>**Root Cause**: OAS inflation indexing likely not capped at legislated maximum benefit amount |
| **Acceptance Criteria** | - [ ] OAS benefits capped at legislated maximum for each year<br>- [ ] Maximum benefit amount indexed to inflation each year (e.g., 2% per year)<br>- [ ] Cap applied AFTER inflation indexing and BEFORE clawback<br>- [ ] Tax configuration includes OAS maximum benefit per year<br>- [ ] Historical maximums documented (2025: ~$8,500, 2026: ~$8,670, etc.)<br>- [ ] Test case: OAS starting at $8,000 in 2025 → max $8,500 in 2025, max $8,670 in 2026<br>- [ ] Test case: OAS cannot exceed ~$14,000 in 2050 (assuming 2% annual cap increase)<br>- [ ] All 6 test accounts pass OAS maximum validation<br>- [ ] OAS clawback still applies correctly (clawback calculated on capped amount)<br>- [ ] No regression - OAS amounts below maximum remain unchanged |
| **Tasks** | - [ ] **Research OAS Maximums** (1 hour):<br>  - Find CRA/Service Canada OAS maximum benefit amounts for 2025-2026<br>  - Document historical maximum amounts (2020-2025)<br>  - Determine indexing rate for maximum (typically matches inflation, ~2%/year)<br>  - Note: OAS has additional 10% top-up for age 75+ (max $9,350 in 2025)<br>- [ ] **Update Tax Configuration** (1 hour):<br>  - Add `oas_max_annual_benefit` to tax_config_canada_2025.json<br>  - Add `oas_max_benefit_age_75_topup` (10% increase at age 75)<br>  - Add `oas_max_benefit_indexing_rate` (default 2.0%)<br>  - Document assumptions in tax config<br>- [ ] **Implement OAS Cap** (1 hour):<br>  - Open `juan-retirement-app/modules/simulation.py`<br>  - Locate OAS benefit calculation<br>  - After applying inflation indexing, apply maximum cap<br>  - Apply age 75+ top-up AFTER cap (10% increase)<br>  - Then apply OAS clawback if income exceeds threshold<br>  - Formula: `oas_benefit = min(oas_after_inflation, oas_max_for_year)`<br>  - Calculate `oas_max_for_year` based on base year maximum + inflation + age adjustment<br>- [ ] **Test Implementation** (1 hour):<br>  - Re-run calculation validation tests on all 6 accounts<br>  - Verify OAS benefits never exceed maximum in any year<br>  - Verify OAS amounts reasonable in 2050, 2070, 2090<br>  - Verify age 75+ top-up applied correctly (10% more than base max)<br>  - Test edge cases:<br>    - User starts OAS at maximum ($8.5K in 2025)<br>    - User starts OAS below maximum ($6K)<br>    - User age 75+ receives top-up<br>    - User subject to OAS clawback (high income)<br>- [ ] **Documentation** (30 min):<br>  - Document OAS maximum amounts in tax_config comments<br>  - Update API documentation<br>  - Add note to OAS calculation docs |
| **Technical Notes** | **OAS Maximum Benefit Amounts (Service Canada):**<br>```<br>Year 2025: $8,506 (age 65-74)<br>Year 2025: $9,357 (age 75+, 10% top-up)<br>Year 2024: $8,216<br>Year 2023: $7,707<br>Year 2022: $7,362<br>Year 2021: $7,192<br><br>Average annual increase: ~4% (2021-2025)<br>Conservative estimate: ~2-3% per year going forward<br>```<br><br>**Tax Config Update (tax_config_canada_2025.json):**<br>```json<br>{<br>  "oas": {<br>    "max_annual_benefit_2025": 8506,<br>    "max_benefit_age_75_topup_percent": 10.0,<br>    "max_benefit_indexing_rate": 2.0,<br>    "notes": "OAS maximum benefit is indexed quarterly to CPI. Using 2% annual average. Age 75+ receive 10% top-up."<br>  }<br>}<br>```<br><br>**Proposed Implementation (simulation.py):**<br>```python<br>def calculate_oas_with_cap(base_oas, start_year, current_year, age, tax_config):<br>    """<br>    Calculate OAS benefit with inflation indexing, maximum cap, AND age 75+ top-up.<br>    """<br>    # Get base maximum and indexing rate from config<br>    oas_max_2025 = tax_config['oas']['max_annual_benefit_2025']<br>    topup_percent = tax_config['oas']['max_benefit_age_75_topup_percent']<br>    indexing_rate = tax_config['oas']['max_benefit_indexing_rate'] / 100.0<br>    <br>    # Calculate years since 2025<br>    years_since_2025 = current_year - 2025<br>    <br>    # Calculate maximum for current year (compound indexing)<br>    oas_max_current_year = oas_max_2025 * ((1 + indexing_rate) ** years_since_2025)<br>    <br>    # Apply age 75+ top-up to maximum<br>    if age >= 75:<br>        oas_max_current_year *= (1 + topup_percent / 100.0)<br>    <br>    # Apply inflation to base OAS (existing logic)<br>    oas_after_inflation = apply_inflation_to_oas(base_oas, start_year, current_year)<br>    <br>    # Apply age 75+ top-up to inflated amount<br>    if age >= 75:<br>        oas_after_inflation *= (1 + topup_percent / 100.0)<br>    <br>    # Apply cap<br>    oas_capped = min(oas_after_inflation, oas_max_current_year)<br>    <br>    return oas_capped<br>```<br><br>**Example - OAS Starting at $8K in 2025 (Age 65):**<br>```<br>2025 (Age 65): $8,000 (< $8,506 max) → $8,000<br>2030 (Age 70): $8,000 * 1.02^5 = $8,833 (< $9,390 max) → $8,833<br>2036 (Age 76): $8,000 * 1.02^11 * 1.10 = $10,676 (< $11,538 max age 75+) → $10,676<br>2050 (Age 90): $8,000 * 1.02^25 * 1.10 = $27,060 (> $15,400 max age 75+) → $15,400 (CAPPED)<br>```<br><br>**Example - OAS with Clawback (High Income):**<br>```<br>2025 (Age 67): <br>  Base OAS: $8,500 (at max)<br>  Income: $100,000<br>  OAS Threshold: $90,997<br>  Excess: $9,003<br>  Clawback: $9,003 * 15% = $1,350<br>  Net OAS: $8,500 - $1,350 = $7,150<br>```<br><br>**Files to Modify:**<br>- `juan-retirement-app/tax_config_canada_2025.json` - Add OAS maximum<br>- `juan-retirement-app/modules/simulation.py` - Implement OAS cap logic<br>- `juan-retirement-app/docs/CALCULATION_NOTES.md` - Document OAS cap |
| **User Impact** | **MEDIUM - Long-term Accuracy**<br>- Primarily affects long-term projections (25+ years into retirement)<br>- Overestimating OAS leads to underestimating need to withdraw from savings<br>- Particularly problematic for low-income users relying heavily on OAS+GIS<br>- Could affect GIS eligibility calculations if OAS amounts are inflated |
| **Priority Justification** | **P1 (High)** because:<br>1. **Data accuracy** - Projections should use realistic government benefit amounts<br>2. **100% affected** - All 6 test accounts show OAS exceeding maximums in later years<br>3. **Quick fix** - 3 story points, ~4 hours total<br>4. **Low risk** - Only affects OAS calculation, isolated change<br>5. **Same pattern as US-081** - Can reuse CPP cap implementation approach |
| **Success Metrics** | - [ ] All 6 test accounts pass OAS maximum validation (0% → 100%)<br>- [ ] OAS benefits in year 2050 reasonable (< $15,000)<br>- [ ] OAS benefits in year 2070 reasonable (< $22,000)<br>- [ ] Age 75+ top-up applied correctly (10% more than base max)<br>- [ ] OAS clawback still works correctly with capped amounts<br>- [ ] No regression - OAS amounts below maximum unchanged<br>- [ ] Documentation clearly explains OAS maximum, age 75+ top-up, and indexing |
| **Related Stories** | - US-081 (Cap CPP at Maximum) - Same issue, same solution pattern<br>- US-038 (CPP/OAS Income Timing Bug) - Related to OAS calculations<br>- US-011 (GIS Strategy) - OAS affects GIS eligibility<br>- US-080 (RRIF Minimums) - Other calculation accuracy issue |
| **Test Evidence** | **Source**: CALCULATION_VALIDATION_TEST_RESULTS.md (February 5, 2026)<br><br>**Example Failure (test@example.com):**<br>- Year 2050 (Age 69): OAS = $12,063<br>  - Expected Maximum (2050): ~$13,800 (2025 $8.5K * 1.02^25)<br>  - Result: ⚠️ WARN - Exceeds 2025 maximum, likely within projected 2050 maximum<br><br>- Year 2076 (Age 95): OAS = $20,187<br>  - Expected Maximum (2076): ~$20,500 (2025 $8.5K * 1.02^51 * 1.10 age 75+ top-up)<br>  - Result: ⚠️ WARN - Close to projected maximum, may be reasonable<br>  - Needs verification: If this includes age 75+ top-up, may be correct<br><br>**Affected Accounts**: 6/6 (100%) in later retirement years |

| ID | User Story | Story Points | Priority | Status |
|----|------------|--------------|----------|--------|
| **US-083** | **Add Basic Personal Amount to Simulation Output** | **2** | **P2** 🟢 | **📋 To Do** |
| **Description** | As a user analyzing detailed tax calculations, I want to see the Basic Personal Amount (BPA) tax credit in the simulation output dataframe so that I can understand how non-refundable tax credits reduce my taxes |
| **Bug Report** | **Discovery**: February 5, 2026 - Comprehensive calculation validation testing<br>**Issue**: Basic Personal Amount (BPA) tax credit not included in simulation output dataframe<br>**Impact**: Missing data for detailed tax analysis, graph/table generation may be incomplete<br>**Evidence**: All 6 test accounts show BPA credit missing from year-by-year results<br>**Current State**: BPA likely applied internally during tax calculation (taxes are correct), but value not exposed in output<br>**Root Cause**: `YearResult` dataclass or output filtering likely not including BPA credit field |
| **Acceptance Criteria** | - [ ] BPA credit amount included in simulation output dataframe<br>- [ ] Column name: `basic_personal_amount_credit` (federal tax credit)<br>- [ ] Value calculated correctly: (BPA * lowest_tax_rate) for federal<br>- [ ] Provincial BPA credit also included: `provincial_bpa_credit`<br>- [ ] Test case: 2025 federal BPA $15,705 → credit $2,356 (15% rate)<br>- [ ] Test case: 2025 ON provincial BPA $11,865 → credit $593 (5% rate)<br>- [ ] Documentation explains BPA credit in output schema<br>- [ ] Graph/table components can use BPA data if needed<br>- [ ] No change to tax calculation logic (already works correctly) |
| **Tasks** | - [ ] **Locate Tax Credit Calculation** (30 min):<br>  - Open `juan-retirement-app/modules/simulation.py`<br>  - Find federal and provincial tax calculation functions<br>  - Identify where BPA credit is calculated<br>  - Confirm BPA is already being applied to reduce taxes<br>- [ ] **Add BPA to YearResult** (1 hour):<br>  - Open `juan-retirement-app/modules/models.py` (or wherever YearResult defined)<br>  - Add fields to YearResult dataclass:<br>    - `basic_personal_amount_credit: float = 0.0`<br>    - `provincial_bpa_credit: float = 0.0`<br>  - Update simulation.py to populate these fields<br>  - Store BPA credit value in year result after tax calculation<br>- [ ] **Update Output Schema** (30 min):<br>  - Ensure YearResult → DataFrame conversion includes new fields<br>  - Verify columns appear in simulation results JSON<br>  - Update TypeScript types if needed (webapp/lib/types/simulation.ts)<br>- [ ] **Test Implementation** (30 min):<br>  - Re-run calculation validation tests<br>  - Verify BPA credit appears in output for all 6 accounts<br>  - Verify BPA values reasonable (e.g., $2,356 federal in 2025)<br>  - Verify no regression in tax calculations<br>- [ ] **Documentation** (30 min):<br>  - Update API_REFERENCE.md with new output fields<br>  - Document BPA credit formula<br>  - Add example output showing BPA credit |
| **Technical Notes** | **Basic Personal Amount (2025):**<br>```<br>Federal BPA: $15,705<br>Federal Tax Rate (lowest bracket): 15%<br>Federal BPA Credit: $15,705 * 15% = $2,356<br><br>Provincial BPA (Ontario): $11,865<br>Provincial Tax Rate (lowest bracket): 5.05%<br>Provincial BPA Credit: $11,865 * 5.05% = $599<br><br>Total BPA Credit: $2,356 + $599 = $2,955<br>```<br><br>**YearResult Dataclass Update (models.py):**<br>```python<br>@dataclass<br>class YearResult:<br>    year: int<br>    age_p1: int<br>    # ... existing fields ...<br>    federal_tax: float<br>    provincial_tax: float<br>    <br>    # NEW FIELDS<br>    basic_personal_amount_credit: float = 0.0  # Federal BPA credit<br>    provincial_bpa_credit: float = 0.0          # Provincial BPA credit<br>    <br>    # ... other fields ...<br>```<br><br>**Simulation Update (simulation.py):**<br>```python<br>def calculate_federal_tax(taxable_income, tax_config):<br>    # ... calculate tax brackets ...<br>    <br>    # Calculate BPA credit<br>    bpa_amount = tax_config['federal']['basic_personal_amount']<br>    lowest_rate = tax_config['federal']['tax_brackets'][0]['rate']<br>    bpa_credit = bpa_amount * lowest_rate<br>    <br>    # Apply credit to reduce tax<br>    tax_after_credits = max(0, tax_before_credits - bpa_credit)<br>    <br>    return {<br>        'tax': tax_after_credits,<br>        'bpa_credit': bpa_credit  # RETURN THIS VALUE<br>    }<br><br># In simulate_year() function:<br>federal_tax_result = calculate_federal_tax(taxable_income, tax_config)<br>provincial_tax_result = calculate_provincial_tax(taxable_income, province, tax_config)<br><br>year_result = YearResult(<br>    year=current_year,<br>    # ... other fields ...<br>    federal_tax=federal_tax_result['tax'],<br>    provincial_tax=provincial_tax_result['tax'],<br>    basic_personal_amount_credit=federal_tax_result['bpa_credit'],  # NEW<br>    provincial_bpa_credit=provincial_tax_result['bpa_credit'],       # NEW<br>    # ... other fields ...<br>)<br>```<br><br>**Tax Config (tax_config_canada_2025.json):**<br>```json<br>{<br>  "federal": {<br>    "basic_personal_amount": 15705,<br>    "tax_brackets": [<br>      {"limit": 55867, "rate": 0.15},<br>      ...<br>    ]<br>  },<br>  "provinces": {<br>    "ON": {<br>      "basic_personal_amount": 11865,<br>      "tax_brackets": [<br>        {"limit": 51446, "rate": 0.0505},<br>        ...<br>      ]<br>    }<br>  }<br>}<br>```<br><br>**Files to Modify:**<br>- `juan-retirement-app/modules/models.py` - Add BPA fields to YearResult<br>- `juan-retirement-app/modules/simulation.py` - Populate BPA fields<br>- `webapp/lib/types/simulation.ts` - Add BPA to TypeScript types (if needed)<br>- `webapp/docs/API_REFERENCE.md` - Document BPA fields |
| **User Impact** | **LOW - Data Completeness**<br>- No immediate calculation impact (BPA already applied correctly internally)<br>- Enables more detailed tax analysis and reporting<br>- Helpful for users who want to understand tax credit breakdown<br>- Improves transparency of tax calculations |
| **Priority Justification** | **P2 (Medium)** because:<br>1. **No calculation impact** - Taxes already correct, just missing data exposure<br>2. **Nice to have** - Improves data completeness but not critical<br>3. **Quick fix** - 2 story points, ~2.5 hours total<br>4. **Low risk** - Only adds fields to output, no logic changes<br>5. **Lower priority than P0/P1 issues** - US-080, US-081, US-082 more critical |
| **Success Metrics** | - [ ] All 6 test accounts pass BPA credit output validation (0% → 100%)<br>- [ ] BPA credit appears in year-by-year results<br>- [ ] BPA values match expected amounts (e.g., $2,356 federal in 2025)<br>- [ ] No regression - tax calculations still correct<br>- [ ] Documentation clearly explains BPA credit fields |
| **Related Stories** | - US-084 (Add Age Credit to Output) - Same pattern, same solution<br>- US-080 (RRIF Minimums) - Both improve calculation transparency |
| **Test Evidence** | **Source**: CALCULATION_VALIDATION_TEST_RESULTS.md (February 5, 2026)<br><br>**Missing Field**: `basic_personal_amount_credit`<br>**Affected Accounts**: 6/6 (100%)<br>**Impact**: Missing data for detailed tax analysis, but tax calculations still correct |

| ID | User Story | Story Points | Priority | Status |
|----|------------|--------------|----------|--------|
| **US-084** | **Add Age Credit to Simulation Output** | **2** | **P2** 🟢 | **📋 To Do** |
| **Description** | As a senior user (age 65+) analyzing detailed tax calculations, I want to see the Age Amount tax credit in the simulation output dataframe so that I can understand how senior tax credits reduce my taxes |
| **Bug Report** | **Discovery**: February 5, 2026 - Comprehensive calculation validation testing<br>**Issue**: Age Amount tax credit (for seniors 65+) not included in simulation output dataframe<br>**Impact**: Missing data for detailed tax credit analysis, graph/table generation may be incomplete<br>**Evidence**: All 6 test accounts show Age credit missing from year-by-year results for ages 65+<br>**Current State**: Age credit likely applied internally during tax calculation (taxes are correct), but value not exposed in output<br>**Root Cause**: `YearResult` dataclass or output filtering likely not including age credit field |
| **Acceptance Criteria** | - [ ] Age credit amount included in simulation output dataframe for ages 65+<br>- [ ] Column name: `age_amount_credit` (federal tax credit)<br>- [ ] Value calculated correctly: (Age Amount * lowest_tax_rate) for federal<br>- [ ] Age credit subject to income clawback (net income > $43,790 in 2025)<br>- [ ] Provincial age credit also included where applicable: `provincial_age_credit`<br>- [ ] Test case: 2025 federal Age Amount $8,790 → credit $1,319 (15% rate) if income < $43,790<br>- [ ] Test case: High income reduces age credit (clawback 15% of excess income)<br>- [ ] Age credit = $0 for ages < 65<br>- [ ] Documentation explains age credit calculation and clawback<br>- [ ] No change to tax calculation logic (already works correctly) |
| **Tasks** | - [ ] **Locate Age Credit Calculation** (30 min):<br>  - Open `juan-retirement-app/modules/simulation.py`<br>  - Find federal and provincial tax calculation functions<br>  - Identify where age credit is calculated<br>  - Confirm age credit is already being applied to reduce taxes for seniors<br>  - Document clawback logic (income threshold, reduction rate)<br>- [ ] **Add Age Credit to YearResult** (1 hour):<br>  - Open `juan-retirement-app/modules/models.py` (or wherever YearResult defined)<br>  - Add fields to YearResult dataclass:<br>    - `age_amount_credit: float = 0.0`<br>    - `provincial_age_credit: float = 0.0`<br>  - Update simulation.py to populate these fields<br>  - Store age credit value in year result after tax calculation<br>  - Handle age < 65: credit = $0<br>  - Handle income clawback: reduce credit if income > threshold<br>- [ ] **Update Output Schema** (30 min):<br>  - Ensure YearResult → DataFrame conversion includes new fields<br>  - Verify columns appear in simulation results JSON<br>  - Update TypeScript types if needed (webapp/lib/types/simulation.ts)<br>- [ ] **Test Implementation** (30 min):<br>  - Re-run calculation validation tests<br>  - Verify age credit appears in output for all 6 accounts at age 65+<br>  - Verify age credit = $0 for ages < 65<br>  - Verify age credit values reasonable (e.g., $1,319 federal in 2025 for low income)<br>  - Test clawback: High income seniors should have reduced/zero age credit<br>  - Verify no regression in tax calculations<br>- [ ] **Documentation** (30 min):<br>  - Update API_REFERENCE.md with new output fields<br>  - Document age credit formula and clawback logic<br>  - Add example output showing age credit for different income levels |
| **Technical Notes** | **Age Amount Tax Credit (2025):**<br>```<br>Federal Age Amount: $8,790 (age 65+)<br>Federal Tax Rate (lowest bracket): 15%<br>Federal Age Credit (before clawback): $8,790 * 15% = $1,319<br><br>Income Threshold: $43,790<br>Clawback Rate: 15% of excess net income<br><br>Example 1 - Low Income ($30K net income):<br>  Net Income: $30,000 (< $43,790 threshold)<br>  Age Amount: $8,790 (no reduction)<br>  Age Credit: $1,319<br><br>Example 2 - Medium Income ($50K net income):<br>  Net Income: $50,000<br>  Excess: $50,000 - $43,790 = $6,210<br>  Reduction: $6,210 * 15% = $932<br>  Age Amount: $8,790 - $932 = $7,858<br>  Age Credit: $7,858 * 15% = $1,179<br><br>Example 3 - High Income ($102K net income):<br>  Net Income: $102,000<br>  Excess: $102,000 - $43,790 = $58,210<br>  Reduction: $8,790 (fully clawed back, excess > $58,600)<br>  Age Amount: $0<br>  Age Credit: $0<br>```<br><br>**Provincial Age Credit (Ontario 2025):**<br>```<br>Ontario Age Amount: $5,775 (age 65+)<br>Ontario Tax Rate (lowest bracket): 5.05%<br>Ontario Age Credit (before clawback): $5,775 * 5.05% = $292<br><br>Ontario has similar income-based reduction<br>```<br><br>**YearResult Dataclass Update (models.py):**<br>```python<br>@dataclass<br>class YearResult:<br>    year: int<br>    age_p1: int<br>    # ... existing fields ...<br>    federal_tax: float<br>    provincial_tax: float<br>    <br>    # NEW FIELDS<br>    age_amount_credit: float = 0.0       # Federal age credit<br>    provincial_age_credit: float = 0.0   # Provincial age credit<br>    <br>    # ... other fields ...<br>```<br><br>**Simulation Update (simulation.py):**<br>```python<br>def calculate_age_credit(age, net_income, tax_config):<br>    """<br>    Calculate federal age amount tax credit.<br>    Applies to ages 65+ with income-based clawback.<br>    """<br>    # No credit if under 65<br>    if age < 65:<br>        return 0.0<br>    <br>    # Get age amount and threshold from config<br>    age_amount_max = tax_config['federal']['age_amount']<br>    threshold = tax_config['federal']['age_amount_threshold']<br>    clawback_rate = 0.15<br>    <br>    # Calculate reduction based on income<br>    if net_income > threshold:<br>        excess = net_income - threshold<br>        reduction = min(excess * clawback_rate, age_amount_max)<br>        age_amount = max(0, age_amount_max - reduction)<br>    else:<br>        age_amount = age_amount_max<br>    <br>    # Calculate credit (age amount * lowest tax rate)<br>    lowest_rate = tax_config['federal']['tax_brackets'][0]['rate']<br>    age_credit = age_amount * lowest_rate<br>    <br>    return age_credit<br><br># In simulate_year() function:<br>federal_age_credit = calculate_age_credit(current_age_p1, net_income_p1, tax_config)<br>provincial_age_credit = calculate_provincial_age_credit(current_age_p1, net_income_p1, province, tax_config)<br><br>year_result = YearResult(<br>    year=current_year,<br>    # ... other fields ...<br>    age_amount_credit=federal_age_credit,      # NEW<br>    provincial_age_credit=provincial_age_credit, # NEW<br>    # ... other fields ...<br>)<br>```<br><br>**Tax Config (tax_config_canada_2025.json):**<br>```json<br>{<br>  "federal": {<br>    "age_amount": 8790,<br>    "age_amount_threshold": 43790,<br>    "age_amount_clawback_rate": 0.15<br>  },<br>  "provinces": {<br>    "ON": {<br>      "age_amount": 5775,<br>      "age_amount_threshold": 43790,<br>      "age_amount_clawback_rate": 0.05<br>    }<br>  }<br>}<br>```<br><br>**Files to Modify:**<br>- `juan-retirement-app/modules/models.py` - Add age credit fields to YearResult<br>- `juan-retirement-app/modules/simulation.py` - Populate age credit fields<br>- `juan-retirement-app/tax_config_canada_2025.json` - Ensure age amount values present<br>- `webapp/lib/types/simulation.ts` - Add age credit to TypeScript types (if needed)<br>- `webapp/docs/API_REFERENCE.md` - Document age credit fields |
| **User Impact** | **LOW - Data Completeness**<br>- No immediate calculation impact (age credit already applied correctly internally)<br>- Enables more detailed tax analysis for seniors<br>- Helpful for users age 65+ who want to understand tax credit breakdown<br>- Improves transparency of senior tax benefits |
| **Priority Justification** | **P2 (Medium)** because:<br>1. **No calculation impact** - Taxes already correct, just missing data exposure<br>2. **Nice to have** - Improves data completeness but not critical<br>3. **Quick fix** - 2 story points, ~2.5 hours total<br>4. **Low risk** - Only adds fields to output, no logic changes<br>5. **Same pattern as US-083** - Can implement both together efficiently<br>6. **Lower priority than P0/P1 issues** - US-080, US-081, US-082 more critical |
| **Success Metrics** | - [ ] All 6 test accounts pass age credit output validation (0% → 100%)<br>- [ ] Age credit appears in year-by-year results for ages 65+<br>- [ ] Age credit = $0 for ages < 65<br>- [ ] Age credit values match expected amounts (e.g., $1,319 federal for low income in 2025)<br>- [ ] Age credit reduced/zero for high income seniors (clawback working)<br>- [ ] No regression - tax calculations still correct<br>- [ ] Documentation clearly explains age credit calculation and clawback |
| **Related Stories** | - US-083 (Add BPA to Output) - Same pattern, same solution<br>- US-080 (RRIF Minimums) - Both improve calculation transparency<br>- US-011 (GIS Strategy) - Age credit affects net income, which affects GIS |
| **Test Evidence** | **Source**: CALCULATION_VALIDATION_TEST_RESULTS.md (February 5, 2026)<br><br>**Missing Field**: `age_amount_credit`<br>**Affected Accounts**: 6/6 (100%) for ages 65+<br>**Impact**: Missing data for detailed tax analysis, but tax calculations still correct |
...
| ID | User Story | Story Points | Priority | Status |
|----|------------|--------------|----------|--------|
| **US-044** | **Fix Cash Flow Gap Detection with High Asset Balances** | **5** | **P0** | **📋 To Do** |
...
| **Next Steps** | 1. ✅ Create user story (US-044) and add to backlog<br>2. Query juanclavierb@gmail.com scenario data<br>3. Reproduce gap in test environment<br>4. Investigate withdrawal strategy code<br>5. Identify and fix root cause<br>6. Deploy fix to production<br>7. Verify with user scenario |
...
| ID | User Story | Story Points | Priority | Status |
|----|------------|--------------|----------|--------|
| **US-043** | **Mobile: Change Withdrawal Strategy After Simulation Results** | **3** | **P2** | **📋 To Do** |
...
| **Success Metrics** | - [ ] Strategy selector visible on mobile devices (100% of screen sizes)<br>- [ ] Touch target size ≥ 44x44px (iOS/Android guidelines met)<br>- [ ] Strategy change triggers re-simulation in < 1 second<br>- [ ] 0 user complaints about mobile strategy switching<br>- [ ] Mobile conversion rate (strategy changes) equals desktop rate<br>- [ ] User testing shows >90% can successfully change strategies on mobile<br>- [ ] Works on iOS Safari, Android Chrome, mobile Firefox |
| **User Journey** | **Before (Current - Broken)**:<br>1. User runs simulation on mobile<br>2. Views results<br>3. Wants to try different strategy<br>4. ❌ Cannot find strategy selector<br>5. Navigates back to profile or settings<br>6. Changes strategy setting (if exists)<br>7. Re-runs simulation<br>8. Frustration and lost context<br><br>**After (Fixed)**:<br>1. User runs simulation on mobile ✅<br>2. Views results ✅<br>3. Sees current strategy label ("Income Minimization") ✅<br>4. Taps strategy dropdown/selector ✅<br>5. Selects "Balanced Income" ✅<br>6. Simulation automatically re-runs (loading indicator) ✅<br>7. Results update in-place ✅<br>8. Compares new results to previous ✅<br>9. Tries 3-4 different strategies easily ✅<br>10. Makes informed decision ✅ |
...
| **US-045** | **Implement 10-Simulation Limit for Free Tier** | **5** | **P1** | **📋 To Do** |
...
| **US-046** | **Improve Low Success Rate Messaging** | **8** | **P0** | **📋 To Do** |
...
| **US-047** | **Fix Baseline Scenario Auto-Population from User Profile** | **3** | **P1** | **📋 To Do** |
...
| **US-048** | **Add Early Retirement Validation & Warnings** | **5** | **P1** | **📋 To Do** |
...
| **US-049** | **Create Early Retirement Planning Guide** | **2** | **P2** | **📋 To Do** |
...
**Total User Stories**: 38 ⬆️ New: US-036 (RRIF Tax Withholding), US-037 (Real Estate), US-038 (Income Timing Bug), US-039 (Pension Start Dates), US-042 (Strategy Name Alignment), US-043 (Mobile Strategy Switching)
**Completed**: 5 (13%)
...
- P2 (Medium): 12 stories (1 done, 11 to do) ⬆️ New: US-036 (RRIF Withholding), US-037 (Real Estate), US-042 (Strategy Name Alignment), US-043 (Mobile Strategy Switching)
...
- Epic 7 (Performance): 3 stories, 16 pts (all to do) ⬆️ New: US-043 (Mobile Strategy Switching)
