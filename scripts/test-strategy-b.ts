...
async function testStrategyB() {
  ...
  const testPayload = {
    ...
    p1: {
      ...
      tfsa_balance: 183000,
      rrsp_balance: 185000, // RRSP (will convert to RRIF at age 71)
      rrif_balance: 0,
  ...
  try {
    ...
    // Check if RRIF is used first OR if RRIF minimum doesn't apply yet
    const rrifUsedOrNotApplicable = rrifWithdrawal > 100 || year1.age_p1 < 71;
    const corpUsedSecond = corpWithdrawal > 100 && nonregWithdrawal < 100;
    ...
    console.log(
      rrifWithdrawal > 100
        ...
        : year1.age_p1 < 71
        ? '⚠️  RRIF not used (age < 71, RRIF minimum not yet applicable)'
        : '❌ RRIF NOT used first (Strategy B FAILED)'
    );

    console.log(
      corpUsedSecond || (year1.age_p1 < 71 && corpWithdrawal > 100)
        ? '✅ Corporate used as expected'
        : '❌ Corporate NOT used as expected'
    );

    console.log(
      nonregWithdrawal < 100
        ? '✅ NonReg NOT used first (correct for Strategy B)'
        : '⚠️ NonReg used (may indicate RRIF depleted or not applicable)'
    );
